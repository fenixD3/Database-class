...
shared_ptr<Node> ParseComparison(It& current, It end)
{
	...
	Comparison cmp; // It's enum with comparison signs
	if (op.value == "<")
		cmp = Comparison::Less;
	else if (op.value == "<=")
	...
	if (column.value == "date")
	{
		...
		return make_shared<DateComparisonNode>(cmp, ParseDate(is)); // enum's obj - int and Date obj
	}
	else
		return make_shared<EventComparisonNode>(cmp, value); // enum's obj - int and istream
}
...
{
	if (current == end)
		return shared_ptr<Node>(); // if it don't have tokens's vector (empty condition)
	...
	while (current != end && current->type != TokenType::PAREN_RIGHT)
	{
		...
		); // enum's obj, all nodes before last, last node
	}
...
{
	auto tokens = Tokenize(is); // return a vector of tokens vector<Token> (parsed condition with its type)
	auto current = tokens.begin(); // vector<Token>::iterator
	auto top_node = ParseExpression(current, tokens.end(), 0u); // return one of Node's type as shared_ptr

	if (!top_node)
		top_node = make_shared<EmptyNode>(); // if tokens's vector is empty (haven't been a condition)
